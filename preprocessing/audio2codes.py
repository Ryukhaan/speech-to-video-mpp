...
def encode_audio(filename, model, t):
    ...
    for i, _ in enumerate(range(number_of_frames)): #enumerate(tqdm(range(number_of_frames), position=1, leave=False)):
...
if __name__ == "__main__":
    ...
    if os.path.isfile(args.dataset):
        ...
        files = glob.glob(args.dataset + "**/*.wav", recursive=True)
        pbar = tqdm(files, position=0)
        for audio in pbar:
            if os.path.isdir(audio): continue
