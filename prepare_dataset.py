...
def get_full_mels(dataset, idx, imgs, full_frames, lm, fps):
    ...
    if np.isnan(orig_mel.reshape(-1)).sum() > 0:
        raise ValueError(
    ...
    while True:
        ...
        i += 1
    print(np.asarray(mel_chunks).shape)
    print(indiv_mels.shape)
    print(mel_chunks_crop.shape)
...
def preprocess(dataset, args):
    ...
    for idx in tqdm(range(len(dataset))):
        ...
        np.save(dataset[idx].split('.')[0] + '_in_img_batch.npy', img_batch)
        # Save Mel-Spec Batch for loss
        np.save(dataset[idx].split('.')[0] + '_mel_batch.npy', mel_batch)
        # Save Indiv Mel Batch
        np.save(dataset[idx].split('.')[0] + '_indiv_mel_batch.npy', mel_batch)
