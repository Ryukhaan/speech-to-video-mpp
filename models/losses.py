...
class LNetLoss(torch.nn.Module):
    ...
    def forward(self, face_pred, face_true, audio_seq, T=5):
        ...
        y_pred = torch.zeros((face_pred.shape[0], face_pred.shape[1], face_pred.shape[2], 384,384))
        y_true = torch.zeros((face_true.shape[0], face_true.shape[1], face_true.shape[2], 384,384))
        for i in range(T):
        ...
        # L_sync = 0.0
        lsync_val = self.lip_sync_loss(audio_seq.squeeze(1), y_pred, y_true)
