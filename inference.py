...
def main():
    ...
    for i, (img_batch, mel_batch, frames, coords, img_original, f_frames) in enumerate(
            tqdm(gen, desc='[Step 6] Lip Synthesis:', total=int(np.ceil(float(delta) / args.LNet_batch_size)))):
        ...
        with torch.no_grad():
            ...
            for idx, lr in enumerate(low_res):
                out_low.write(np.uint8(lr.cpu().numpy().transpose(1,2,0) * 255))
            ...
            if args.up_face == 'original':
                ...
                test_batch = {'src_img': torch.nn.functional.interpolate((img_original * 2 - 1), size=(128, 128),
                                                                         mode='bilinear'),
                              'tar_aus': tar_aus.repeat(len(incomplete), 1)}
                instance.feed_batch(test_batch)
                instance.forward()
                cur_gen_faces = torch.nn.functional.interpolate(instance.fake_img / 2. + 0.5, size=(384, 384),
                                                                mode='bilinear')
