...
def main():    
    ...
    #mel_chunks = mel_chunks[:20] # Change here length of inference video
    ...
    for i, (img_batch, mel_batch, frames, coords, img_original, f_frames) in enumerate(tqdm(gen, desc='[Step 6] Lip Synthesis:', total=int(np.ceil(float(len(mel_chunks)) / args.LNet_batch_size)))):
        ...
        for p, f, xf, c in zip(pred, frames, f_frames, coords):
            ...
            cv2.imwrite("./results/low_res{}.png".format(idx), p)
            ...
            if args.cropped_image:
                ...
                bottom_mask = cv2.dilate(bottom_mask, np.array([[0, 1, 0], [0, 1, 0], [0, 0, 0]], dtype=np.uint8), iterations=140)
            ...
            if args.cropped_image:
                ...
                pp, orig_faces, enhanced_faces = enhancer.process_withmask(pp, xf, mask=mask,
                                                                           ...
                                                                           mm=mm)
                pp = cv2.resize(pp, (frame_w, frame_h))
            else:
