...
def main():    
    ...
    mel_chunks = mel_chunks[:60] # Change here length of inference video
        ...
        for p, f, xf, c in zip(pred, frames, f_frames, coords):
            ...
            if args.cropped_image:
                ...
                for j, (x,y) in enumerate(nose):
                    yi, xi = int(inverse_scale_x * x + x1), int(inverse_scale_y * y + y1)
                    yj, xj = int(inverse_scale_x * nose[j - 1][0] + x1), int(inverse_scale_y * nose[j - 1][1] + y1)
                ...
                for j, (x,y) in enumerate(bottom_face):
                    yi, xi = int(inverse_scale_x * x + x1), int(inverse_scale_y * y + y1)
                    yj, xj = int(inverse_scale_x * bottom_face[j - 1][0] + x1), int(inverse_scale_y * bottom_face[j - 1][1] + y1)
